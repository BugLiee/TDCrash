...
static int tacc_unwind_clang_record_frame(tacc_unwind_clang_t *self, uintptr_t pc)
{
    Dl_info info;
    size_t  len;
    ...
    {
        len = tacc_fmt_snprintf(self->buf + self->buf_used, self->buf_len - self->buf_used,
                               "    #%02zu pc %0"TACC_UTIL_FMT_ADDR"  <unknown>\n",
                self->frame_num, pc);
    }
    else
    {
        ...
        {
            len = tacc_fmt_snprintf(self->buf + self->buf_used, self->buf_len - self->buf_used,
                                   "    #%02zu pc %0"TACC_UTIL_FMT_ADDR"  <anonymous:%"TACC_UTIL_FMT_ADDR">\n",
        ...
        else
        {
            ...
            {
                len = tacc_fmt_snprintf(self->buf + self->buf_used, self->buf_len - self->buf_used,
                                       "    #%02zu pc %0"TACC_UTIL_FMT_ADDR"  %s\n",
            ...
            else
            {
                ...
                {
                    len = tacc_fmt_snprintf(self->buf + self->buf_used, self->buf_len - self->buf_used,
                                           "    #%02zu pc %0"TACC_UTIL_FMT_ADDR"  %s (%s)\n",
                            self->frame_num, pc - (uintptr_t)info.dli_fbase, info.dli_fname,
                            info.dli_sname);
                }
                else
                {
                    len = tacc_fmt_snprintf(self->buf + self->buf_used, self->buf_len - self->buf_used,
                                           "    #%02zu pc %0"TACC_UTIL_FMT_ADDR"  %s (%s+%"PRIuPTR")\n",
                            self->frame_num, pc - (uintptr_t)info.dli_fbase, info.dli_fname,
    ...
    self->buf_used += len;
    if(self->buf_len - self->buf_used < 20) return TAC_ERRNO_NOSPACE;

    //check unwinding limit
    self->frame_num++;
    if(self->frame_num >= MAX_FRAMES) return TAC_ERRNO_RANGE;

    return 0;
}

static _Unwind_Reason_Code tacc_unwind_clang_callback(struct _Unwind_Context* unw_ctx, void* arg)
{
    tacc_unwind_clang_t *self = (tacc_unwind_clang_t *)arg;
    ...
    if(0 != tacc_unwind_clang_record_frame(self, pc))
        return _URC_END_OF_STACK; //stop
...
{
    tacc_unwind_clang_t self;

    tacc_libc_support_memset(&self, 0, sizeof(tacc_unwind_clang_t));
    self.uc      = uc;
    self.buf     = buf;
    ...
    _Unwind_Backtrace(tacc_unwind_clang_callback, &self);

    if(0 == self.buf_used)
        tacc_unwind_clang_record_frame(&self, self.sig_pc);
