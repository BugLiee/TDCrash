...
static const char tacc_b64_table[65] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
...
char *tacc_b64_encode(const uint8_t *in, size_t in_len, size_t *out_len)
{
    ...
    while (end - start >= 3)
    {
        *p++ = tacc_b64_table[start[0] >> 2];
        *p++ = tacc_b64_table[((start[0] & 0x03) << 4) | (start[1] >> 4)];
        *p++ = tacc_b64_table[((start[1] & 0x0f) << 2) | (start[2] >> 6)];
        *p++ = tacc_b64_table[start[2] & 0x3f];
        start += 3;
    }
    ...
    if (end - start > 0)
    {
        *p++ = tacc_b64_table[start[0] >> 2];
        if(end - start == 1)
        {
            *p++ = tacc_b64_table[(start[0] & 0x03) << 4];
            *p++ = '=';
        }
        else
        {
            *p++ = tacc_b64_table[((start[0] & 0x03) << 4) | (start[1] >> 4)];
            *p++ = tacc_b64_table[(start[1] & 0x0f) << 2];
        }
        *p++ = '=';
    }
...
uint8_t *tacc_b64_decode(const char *in, size_t in_len, size_t *out_len)
{
    ...
    for(i = 0; i < sizeof(tacc_b64_table) - 1; i++)
        dtable[(size_t)(tacc_b64_table[i])] = (uint8_t)i;
    dtable['='] = 0;
