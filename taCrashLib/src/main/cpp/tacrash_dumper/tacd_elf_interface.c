...
typedef struct tacd_elf_load
{
    uintptr_t vaddr;
    size_t    offset;
    size_t    size;
    TAILQ_ENTRY(tacd_elf_load,) link;
} tacd_elf_load_t;
#pragma clang diagnostic pop
typedef TAILQ_HEAD(tacd_elf_load_queue, tacd_elf_load,) tacd_elf_load_queue_t;

#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wpadded"
typedef struct tacd_elf_symbols
{
    size_t sym_offset;
    size_t sym_end;
    ...
    size_t str_offset;
    size_t str_end;
    TAILQ_ENTRY(tacd_elf_symbols,) link;
} tacd_elf_symbols_t;
#pragma clang diagnostic pop
typedef TAILQ_HEAD(tacd_elf_symbols_queue, tacd_elf_symbols,) tacd_elf_symbols_queue_t;

#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wpadded"
typedef struct tacd_elf_strtab
{
    size_t addr;
    size_t offset;
    TAILQ_ENTRY(tacd_elf_strtab,) link;
} tacd_elf_strtab_t;
#pragma clang diagnostic pop
typedef TAILQ_HEAD(tacd_elf_strtab_queue, tacd_elf_strtab,) tacd_elf_strtab_queue_t;

#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wpadded"
struct tacd_elf_interface
{
    pid_t                    pid;
    ...
    tacd_elf_symbols_queue_t  symbolsq;

    //string tables
    tacd_elf_strtab_queue_t   strtabq;
    ...
    tacd_dwarf_t             *dwarf_eh_frame;
    tacd_dwarf_type_t         dwarf_eh_frame_type;

    //.debug_frame
    size_t                   debug_frame_offset;
    size_t                   debug_frame_size;
    tacd_dwarf_t             *dwarf_debug_frame;
...
static int tacd_elf_interface_check_valid(ElfW(Ehdr) *ehdr)
{
    //check magic
    if(0 != memcmp(ehdr->e_ident, ELFMAG, SELFMAG)) return TAC_ERRNO_FORMAT;

    //check class (64/32)
#if defined(__LP64__)
    if(ELFCLASS64 != ehdr->e_ident[EI_CLASS]) return TAC_ERRNO_FORMAT;
#else
    if(ELFCLASS32 != ehdr->e_ident[EI_CLASS]) return TAC_ERRNO_FORMAT;
#endif

    //check endian (little/big)
    if(ELFDATA2LSB != ehdr->e_ident[EI_DATA]) return TAC_ERRNO_FORMAT;

    //check version
    if(EV_CURRENT != ehdr->e_ident[EI_VERSION]) return TAC_ERRNO_FORMAT;

    //check type
    if(ET_EXEC != ehdr->e_type && ET_DYN != ehdr->e_type) return TAC_ERRNO_FORMAT;

    //check machine
#if defined(__arm__)
    if(EM_ARM != ehdr->e_machine) return TAC_ERRNO_FORMAT;
#elif defined(__aarch64__)
    if(EM_AARCH64 != ehdr->e_machine) return TAC_ERRNO_FORMAT;
#elif defined(__i386__)
    if(EM_386 != ehdr->e_machine) return TAC_ERRNO_FORMAT;
#elif defined(__x86_64__)
    if(EM_X86_64 != ehdr->e_machine) return TAC_ERRNO_FORMAT;
#else
    return TAC_ERRNO_FORMAT;
#endif

    //check version
    if(EV_CURRENT != ehdr->e_version) return TAC_ERRNO_FORMAT;

    return 0;
}

static int tacd_elf_interface_read_program_headers(tacd_elf_interface_t *self, ElfW(Ehdr) *ehdr, uintptr_t *load_bias)
{
    size_t     i;
    ElfW(Phdr) phdr;
    ...
    {
        if(0 != tacd_memory_read_fully(self->memory, ehdr->e_phoff + i, &phdr, sizeof(phdr)))
        {
            r = TAC_ERRNO_MEM;
            goto err;
        }
...
static int tacd_elf_interface_read_section_headers(tacd_elf_interface_t *self, ElfW(Ehdr) *ehdr)
{
    size_t             i;
    ElfW(Shdr)         shdr;
    ElfW(Shdr)         str_shdr;
    size_t             sec_offset = 0;
    size_t             sec_size   = 0;
    tacd_elf_symbols_t *symbols, *symbols_tmp;
    tacd_elf_strtab_t  *strtab, *strtab_tmp;
    char               name[128];
    int                r;
    ...
    {
        if(0 != tacd_memory_read_fully(self->memory, ehdr->e_shoff + ehdr->e_shstrndx * ehdr->e_shentsize,
                                      &shdr, sizeof(shdr))) return TAC_ERRNO_MEM;
    ...
    {
        if(0 != tacd_memory_read_fully(self->memory, ehdr->e_shoff + i, &shdr, sizeof(shdr)))
        {
            r = TAC_ERRNO_MEM;
            goto err;
        }
        ...
        case SHT_NOTE:
            {
                ...
                if(0 != tacd_memory_read_string(self->memory, sec_offset + shdr.sh_name, name, sizeof(name), UINTPTR_MAX))
                    continue;
        ...
        case SHT_DYNSYM:
            {
                ...
                if(0 != tacd_memory_read_fully(self->memory, ehdr->e_shoff + shdr.sh_link * ehdr->e_shentsize, &str_shdr, sizeof(str_shdr)))
                {
                    r = TAC_ERRNO_MEM;
                    goto err;
                }
                ...
                if(NULL == (symbols = malloc(sizeof(tacd_elf_symbols_t))))
                {
                    r = TAC_ERRNO_NOMEM;
                    goto err;
                }
        ...
        case SHT_STRTAB:
            {
                if(NULL == (strtab = malloc(sizeof(tacd_elf_strtab_t))))
                {
                    r = TAC_ERRNO_NOMEM;
                    goto err;
                }
...
int tacd_elf_interface_create(tacd_elf_interface_t **self, pid_t pid, tacd_memory_t *memory, uintptr_t *load_bias)
{
    ...
    //get ELF header
    if(0 != tacd_memory_read_fully(memory, 0, &ehdr, sizeof(ehdr))) return TAC_ERRNO_MEM;
    
    //check valid
    if(0 != (r = tacd_elf_interface_check_valid(&ehdr))) return r;

    //init
    if(NULL == (*self = calloc(1, sizeof(tacd_elf_interface_t)))) return TAC_ERRNO_NOMEM;
    (*self)->pid = pid;
    (*self)->memory = memory;
    ...
    if(0 != (r = tacd_elf_interface_read_program_headers(*self, &ehdr, load_bias)))
    {
        free(*self);
        *self = NULL;
        return r;
    ...
    tacd_elf_interface_read_section_headers(*self, &ehdr);

    //dwarf from .eh_frame with .eh_frame_hdr
    if(0 != (*self)->eh_frame_hdr_offset && 0 != (*self)->eh_frame_hdr_size)
    {
        tacd_dwarf_create(&((*self)->dwarf_eh_frame), memory, pid,
        ...
        (*self)->dwarf_eh_frame_type = TACD_DWARF_TYPE_EH_FRAME_HDR;
    }
    ...
    if(NULL == (*self)->dwarf_eh_frame && 0 != (*self)->eh_frame_offset && 0 != (*self)->eh_frame_size)
    {
        ...
        (*self)->dwarf_eh_frame_type = TACD_DWARF_TYPE_EH_FRAME;
    }

    //dwarf from .debug_frame
    if(0 != (*self)->debug_frame_offset && 0 != (*self)->debug_frame_size)
        tacd_dwarf_create(&((*self)->dwarf_debug_frame), memory, pid,
                         (*self)->load_bias, 0,
...
tacd_elf_interface_t *tacd_elf_interface_gnu_create(tacd_elf_interface_t *self)
{
    ...
    if(0 != tacd_memory_read_fully(self->memory, self->gnu_debugdata_offset, src, src_size)) goto err;

    //xz decompress
    if(0 != tacd_util_xz_decompress(src, src_size, &dst, &dst_size)) goto err;

    //create memory object
    if(0 != tacd_memory_create_from_buf(&memory, dst, dst_size)) goto err;

    //create ELF interface from .gnu_debugdata
    if(0 != tacd_elf_interface_create(&gnu, self->pid, memory, NULL)) goto err;
 ...
 err:
    TACD_LOG_WARN("ELF: create GNU interface FAILED");
    if(NULL != memory) tacd_memory_destroy(&memory);
...
int tacd_elf_interface_dwarf_step(tacd_elf_interface_t *self, uintptr_t step_pc, tacd_regs_t *regs, int *finished)
{
    ...
    {
        r = tacd_dwarf_step(self->dwarf_debug_frame, regs, step_pc, finished);
#if TACD_ELF_INTERFACE_DEBUG
        TACD_LOG_DEBUG("ELF: step by .debug_frame%s %s, step_pc=%"PRIxPTR", load_bias=%"PRIxPTR", finished=%d",
    ...
    {
        r = tacd_dwarf_step(self->dwarf_eh_frame, regs, step_pc, finished);
#if TACD_ELF_INTERFACE_DEBUG
        TACD_LOG_DEBUG("ELF: step by %s%s %s, step_pc=%"PRIxPTR", load_bias=%"PRIxPTR", finished=%d",
                      (TACD_DWARF_TYPE_EH_FRAME_HDR == self->dwarf_eh_frame_type ? ".eh_frame_hdr" : ".eh_frame"),
                      (self->is_gnu ? " (in .gnu_debugdata)" : ""),
                      (0 == r ? "OK" : "FAILED"),
                      step_pc, self->load_bias, *finished);
#endif
...
int tacd_elf_interface_arm_exidx_step(tacd_elf_interface_t *self, uintptr_t step_pc, tacd_regs_t *regs, int *finished)
{
    ...
    {
        r = tacd_arm_exidx_step(regs, self->memory, self->pid, self->arm_exidx_offset, self->arm_exidx_size,
                                  self->load_bias, step_pc, finished);
#if TACD_ELF_INTERFACE_DEBUG
        TACD_LOG_DEBUG("ELF: step by .ARM.exidx %s, step_pc=%x, load_bias=%x, finished=%d",
...
int tacd_elf_interface_get_function_info(tacd_elf_interface_t *self, uintptr_t addr, char **name, size_t *name_offset)
{
    ...
    TAILQ_FOREACH(symbols, &(self->symbolsq), link)
    {
        ...
        {
            if(0 != tacd_memory_read_fully(self->memory, offset, &sym, sizeof(sym))) break;
            ...
            *name_offset = addr - start_offset;
            ...
            if(0 != tacd_memory_read_string(self->memory, str_offset, buf, sizeof(buf), symbols->str_end - str_offset)) continue;
...
int tacd_elf_interface_get_build_id(tacd_elf_interface_t *self, uint8_t *build_id, size_t build_id_len, size_t *build_id_len_ret)
{
    ...
    if(0 == self->build_id_offset || 0 == self->build_id_size) return TAC_ERRNO_MISSING;
    if(self->build_id_size < sizeof(nhdr)) return TAC_ERRNO_FORMAT;

    //read .note.gnu.build-id header
    if(0 != (r = tacd_memory_read_fully(self->memory, self->build_id_offset, &nhdr, sizeof(nhdr)))) return r;
    if(0 == nhdr.n_descsz) return TAC_ERRNO_MISSING;
    if(nhdr.n_descsz > build_id_len) return TAC_ERRNO_NOSPACE;
    ...
    if(0 != (r = tacd_memory_read_fully(self->memory, addr, build_id, nhdr.n_descsz))) return r;
...
char *tacd_elf_interface_get_so_name(tacd_elf_interface_t *self)
{
    ...
    {
        if(0 != tacd_memory_read_fully(self->memory, offset, &dyn, sizeof(dyn))) goto err;
    ...
    TAILQ_FOREACH(strtab, &(self->strtabq), link)
    {
        if(strtab->addr == strtab_addr)
        {
            ...
            if(0 != tacd_memory_read_string(self->memory, soname_offset, buf, sizeof(buf), strtab->offset + strtab_size - soname_offset)) goto err;
