...
static void tdcd_memory_file_uninit(tdcd_memory_file_t *self)
{
    if(NULL != self->data)
    {
...
static int tdcd_memory_file_init(tdcd_memory_file_t *self, size_t size, size_t offset, uint64_t file_size)
{
    tdcd_memory_file_uninit(self);

    if(offset >= (size_t)file_size) return TDC_ERRNO_RANGE;

    size_t aligned_offset = offset & (~(size_t)(getpagesize() - 1));
    if(aligned_offset > (size_t)file_size) return TDC_ERRNO_RANGE;
    ...
    if(map == MAP_FAILED) return TDC_ERRNO_SYS;
...
{
    tdcd_memory_file_t **self = (tdcd_memory_file_t **)obj;
    ...
    if(NULL == map->name || 0 == strlen(map->name)) return TDC_ERRNO_INVAL;
    
    if(NULL == (*self = malloc(sizeof(tdcd_memory_file_t)))) return TDC_ERRNO_NOMEM;
    (*self)->base = base;
    (*self)->fd = -1;
    (*self)->data = NULL;
    (*self)->offset = 0;
...
#pragma clang diagnostic ignored "-Wgnu-statement-expression"
    if(0 > ((*self)->fd = TDCC_UTIL_TEMP_FAILURE_RETRY(open(map->name, O_RDONLY | O_CLOEXEC))))
    {
        r = TDC_ERRNO_SYS;
        goto err;
    }
#pragma clang diagnostic pop
    ...
    if(0 == map->offset)
    {
        if(0 != (r = tdcd_memory_file_init(*self, SIZE_MAX, 0, file_size))) goto err;
        if(!tdcd_elf_is_valid(base))
        {
            r = TDC_ERRNO_MEM;
            goto err;
        }
    ...
    //
    if(0 != (r = tdcd_memory_file_init(*self, map_size, map->offset, file_size))) goto err;
    if(tdcd_elf_is_valid(base))
    {
        map->elf_start_offset = map->offset;
        
        max_size = tdcd_elf_get_max_size(base);
        if(max_size > map_size)
        {
            //try to map the whole file
            if(0 != tdcd_memory_file_init(*self, max_size, map->offset, file_size))
            {
                //rollback
                if(0 != (r = tdcd_memory_file_init(*self, map_size, map->offset, file_size))) goto err;
            }
    ...
    //
    prev_map = tdcd_maps_get_prev_map(maps, map);
       ...
       NULL != prev_map->name && 0 == strcmp(prev_map->name, map->name))
    {
        ...
        if(0 != (r = tdcd_memory_file_init(*self, prev_map_size, prev_map->offset, file_size))) goto err;
        if(tdcd_elf_is_valid(base))
        {
            max_size = tdcd_elf_get_max_size(base);
            if(max_size > prev_map_size)
            {
                if(0 != (r = tdcd_memory_file_init(*self, max_size, prev_map->offset, file_size))) goto err;
...
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wgnu-statement-expression"
    size_t actual_len = TDCC_UTIL_MIN(bytes_left, size);
#pragma clang diagnostic pop
...
const tdcd_memory_handlers_t tdcd_memory_file_handlers = {
    tdcd_memory_file_destroy,
    tdcd_memory_file_read
};
#pragma clang diagnostic pop
