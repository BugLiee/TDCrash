...
typedef struct tdcd_dwarf_cie
{
    ...
    RB_ENTRY(tdcd_dwarf_cie) link;
} tdcd_dwarf_cie_t;
static int tdcd_dwarf_cie_cmp(tdcd_dwarf_cie_t *a, tdcd_dwarf_cie_t *b)
{
    if(a->offset == b->offset) return 0;
    else return (a->offset > b->offset ? 1 : -1);
}
typedef RB_HEAD(tdcd_dwarf_cie_tree, tdcd_dwarf_cie) tdcd_dwarf_cie_tree_t;
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wunused-function"
RB_GENERATE_STATIC(tdcd_dwarf_cie_tree, tdcd_dwarf_cie, link, tdcd_dwarf_cie_cmp)
...
typedef struct
{
    ...
    uintptr_t pc_start;
    uintptr_t pc_end;
    tdcd_dwarf_cie_t *cie;
} tdcd_dwarf_fde_t;

//DWARF object
struct tdcd_dwarf
{
    tdcd_dwarf_type_t          type;
    pid_t                     pid;
    uintptr_t                 load_bias;
    uintptr_t                 hdr_load_bias; //for .eh_frame_hdr
    tdcd_dwarf_cie_tree_t      cie_cache;
    ...
    //TDCD_DWARF_TYPE_DEBUG_FRAME  mode: starting / ending offset of .debug_frame
    //TDCD_DWARF_TYPE_EH_FRAME     mode: starting / ending offset of .eh_frame
    //TDCD_DWARF_TYPE_EH_FRAME_HDR mode: starting / ending offset of binary search table in .eh_frame_hdr
    size_t                    entries_offset;
    size_t                    entries_end;

    //for TDCD_DWARF_TYPE_EH_FRAME_HDR mode only
...
//location rule
#define TDCD_DWARF_REG_NUM 0xFFFF
typedef struct tdcd_dwarf_loc_rule
{
    uint8_t  type;
    uint64_t values[2];
} tdcd_dwarf_loc_rule_t;

//location
typedef struct
{
    tdcd_dwarf_loc_rule_t cfa_rule;
    tdcd_dwarf_loc_rule_t reg_rules[TDCD_DWARF_REG_NUM];
} tdcd_dwarf_loc_t;

//location stack
typedef struct tdcd_dwarf_loc_node
{
    tdcd_dwarf_loc_t loc;
    TAILQ_ENTRY(tdcd_dwarf_loc_node,) link;
} tdcd_dwarf_loc_node_t;
typedef TAILQ_HEAD(tdcd_dwarf_loc_node_stack, tdcd_dwarf_loc_node,) tdcd_dwarf_loc_node_stack_t;
...
static tdcd_dwarf_cfa_t tdcd_dwarf_cfa_table[64] = {
...
static tdcd_dwarf_op_t tdcd_dwarf_op_table[256] = {
...
static int tdcd_dwarf_is_cie_64(tdcd_dwarf_t *self, uint64_t v)
{
    if(TDCD_DWARF_TYPE_DEBUG_FRAME == self->type)
        return (v == (uint64_t)(-1)) ? 1 : 0;
    else
        return (v == 0) ? 1 : 0;
}

static int tdcd_dwarf_is_cie_32(tdcd_dwarf_t *self, uint32_t v)
{
    if(TDCD_DWARF_TYPE_DEBUG_FRAME == self->type)
...
static size_t tdcd_dwarf_adjust_cie_offset(tdcd_dwarf_t *self, size_t cur_field_offset, size_t v)
{
    if(TDCD_DWARF_TYPE_DEBUG_FRAME == self->type)
...
static uintptr_t tdcd_dwarf_adjust_pc_from_fde(tdcd_dwarf_t *self, size_t cur_field_offset, uintptr_t v)
{
    if(TDCD_DWARF_TYPE_DEBUG_FRAME == self->type)
        return v;
    else
...
static int tdcd_dwarf_read_bytes(tdcd_dwarf_t *self, void *value, size_t size)
{
    int r;
    
    if(0 != (r = tdcd_memory_read_fully(self->memory, self->memory_cur_offset, value, size))) return r;
    self->memory_cur_offset += size;
    
    return 0;
}

static int tdcd_dwarf_read_uleb128(tdcd_dwarf_t *self, uint64_t *value)
{
    size_t i;
    int    r;
    
    if(0 != (r = tdcd_memory_read_uleb128(self->memory, self->memory_cur_offset, value, &i))) return r;
    self->memory_cur_offset += i;
    
    return 0;
}

static int tdcd_dwarf_read_sleb128(tdcd_dwarf_t *self, int64_t *value)
{
    size_t i;
    int    r;

    if(0 != (r = tdcd_memory_read_sleb128(self->memory, self->memory_cur_offset, value, &i))) return r;
    self->memory_cur_offset += i;
    
    return 0;
}

static int tdcd_dwarf_read_encoded(tdcd_dwarf_t *self, uint64_t *value, uint8_t encoding)
{
    int       r;
    uintptr_t vp;
    uint8_t   vu8;
    ...
    *value = 0;
    ...
    {
        //check overflow
        if(__builtin_add_overflow(self->memory_cur_offset, sizeof(uintptr_t) - 1, &(self->memory_cur_offset))) return TDC_ERRNO_RANGE;

        //align
        self->memory_cur_offset &= -sizeof(uintptr_t);

        if(0 != (r = tdcd_dwarf_read_bytes(self, &vp, sizeof(uintptr_t)))) return r;
        *value = (uint64_t)vp;
        return 0;
    }
    ...
    case DW_EH_PE_uleb128:
        if(0 != (r = tdcd_dwarf_read_uleb128(self, &vu64))) return r;
        *value = (uint64_t)vu64;
        break;
    case DW_EH_PE_sleb128:
        if(0 != (r = tdcd_dwarf_read_sleb128(self, &vi64))) return r;
        *value = (uint64_t)vi64;
        break;
    case DW_EH_PE_udata1:
        if(0 != (r = tdcd_dwarf_read_bytes(self, &vu8, 1))) return r;
        *value = (uint64_t)vu8;
        break;
    case DW_EH_PE_sdata1:
        if(0 != (r = tdcd_dwarf_read_bytes(self, &vi8, 1))) return r;
        *value = (uint64_t)vi8;
        break;
    case DW_EH_PE_udata2:
        if(0 != (r = tdcd_dwarf_read_bytes(self, &vu16, 2))) return r;
        *value = (uint64_t)vu16;
        break;
    case DW_EH_PE_sdata2:
        if(0 != (r = tdcd_dwarf_read_bytes(self, &vi16, 2))) return r;
        *value = (uint64_t)vi16;
        break;
    case DW_EH_PE_udata4:
        if(0 != (r = tdcd_dwarf_read_bytes(self, &vu32, 4))) return r;
        *value = (uint64_t)vu32;
        break;
    case DW_EH_PE_sdata4:
        if(0 != (r = tdcd_dwarf_read_bytes(self, &vi32, 4))) return r;
        *value = (uint64_t)vi32;
        break;
    case DW_EH_PE_udata8:
        if(0 != (r = tdcd_dwarf_read_bytes(self, &vu64, 8))) return r;
        *value = (uint64_t)vu64;
        break;
    case DW_EH_PE_sdata8:
        if(0 != (r = tdcd_dwarf_read_bytes(self, &vi64, 8))) return r;
        *value = (uint64_t)vi64;
        break;
    default:
    ...
    case DW_EH_PE_absptr:
        return 0;
    case DW_EH_PE_pcrel:
        if(((uintptr_t)-1) == self->memory_pc_offset) return TDC_ERRNO_NOTSPT;
        *value += self->memory_pc_offset;
        return 0;
    case DW_EH_PE_datarel:
        if(((uintptr_t)-1) == self->memory_data_offset) return TDC_ERRNO_NOTSPT;
        *value += self->memory_data_offset;
        return 0;
    case DW_EH_PE_textrel:
    case DW_EH_PE_funcrel:
        return TDC_ERRNO_NOTSPT;
    default:
        return TDC_ERRNO_FORMAT;
    }
}
...
static tdcd_dwarf_cie_t *tdcd_dwarf_get_cie_from_offset(tdcd_dwarf_t *self, size_t offset)
{
    tdcd_dwarf_cie_t  cie_key = {.offset = offset};
    tdcd_dwarf_cie_t *cie = NULL;
    uint8_t          cie_version;
    uint8_t          v8;
    uint32_t         v32;
    uint64_t         v64;
    ...
    //check cache
    if(NULL != (cie = RB_FIND(tdcd_dwarf_cie_tree, &(self->cie_cache), &cie_key))) return cie;
    
    //create cie
    if(NULL == (cie = calloc(1, sizeof(tdcd_dwarf_cie_t)))) goto err;
    cie->offset = offset; //key
    
    //get length
    if(0 != tdcd_dwarf_read_bytes(self, &v32, 4)) goto err;

    if((uint32_t)(-1) == v32) //64bits DWARF FDE
    {
        //get extended length
        if(0 != tdcd_dwarf_read_bytes(self, &v64, 8)) goto err;
        ...
        //get CIE ID
        if(0 != tdcd_dwarf_read_bytes(self, &v64, 8)) goto err;
        if(!tdcd_dwarf_is_cie_64(self, v64)) goto err; //not a CIE
    }
    else //32bits DWARF FDE
    {
        ...
        //get CIE ID
        if(0 != tdcd_dwarf_read_bytes(self, &v32, 4)) goto err;
        if(!tdcd_dwarf_is_cie_32(self, v32)) goto err; //not a CIE
    }

    //check version
    if(0 != tdcd_dwarf_read_bytes(self, &cie_version, 1)) goto err;
    ...
    {
        if(0 != tdcd_dwarf_read_bytes(self, &v8, 1)) goto err;
    ...
    if(4 == cie_version || 5 == cie_version)
    {
        ...
        //get segment size
        if(0 != tdcd_dwarf_read_bytes(self, &(cie->segment_size), 1)) goto err;
    }

    //get code alignment factor
    if(0 != tdcd_dwarf_read_uleb128(self, &(cie->code_alignment_factor))) goto err;

    //get data alignment factor
    if(0 != tdcd_dwarf_read_sleb128(self, &(cie->data_alignment_factor))) goto err;
    ...
    {
        if(0 != tdcd_dwarf_read_uleb128(self, &(cie->return_address_register))) goto err;
    }
    if(cie->return_address_register >= TDCD_REGS_MACHINE_NUM) goto err;
    ...
    else
    {
        ...
        if(0 != tdcd_dwarf_read_uleb128(self, &v64)) goto err;
        ...
        for(i = 1; i < sizeof(cie->augmentation_string); i++)
        {
            ...
            case 'R':
                ...
                if(0 != tdcd_dwarf_read_bytes(self, &(cie->fde_address_encoding), 1)) goto err;
                break;
            case 'P':
                ...
                if(0 != tdcd_dwarf_read_encoded(self, &v64, v8)) goto err;
                break;
            default:
                break;
            }
    ...
    RB_INSERT(tdcd_dwarf_cie_tree, &(self->cie_cache), cie);
    return cie;

 err:
#if TDCD_DWARF_DEBUG
    TDCD_LOG_DEBUG("DWARF: get CIE failed, offset=%"PRIxPTR, offset);
#endif
...
static tdcd_dwarf_fde_t *tdcd_dwarf_get_fde_from_offset(tdcd_dwarf_t *self, size_t *offset, uintptr_t pc)
{
    tdcd_dwarf_fde_t *fde = NULL;
    tdcd_dwarf_cie_t *cie;
    uint64_t         cfa_instructions_offset;
    ...
    //get length
    if(0 != tdcd_dwarf_read_bytes(self, &v32, 4)) goto end;
    
    if((uint32_t)(-1) == v32) //64bits DWARF FDE
    {
        //get extended length
        if(0 != tdcd_dwarf_read_bytes(self, &v64, 8)) goto end;
        ...
        if(0 != tdcd_dwarf_read_bytes(self, &v64, 8)) goto end;
        if(tdcd_dwarf_is_cie_64(self, v64)) goto end; //ignore cie
        if(v64 > SIZE_MAX) goto end;
        cie_offset = tdcd_dwarf_adjust_cie_offset(self, cur_field_offset, (size_t)v64);
    }
    else //32bits DWARF FDE
    {
        ...
        cur_field_offset = self->memory_cur_offset;
        if(0 != tdcd_dwarf_read_bytes(self, &v32, 4)) goto end;
        if(tdcd_dwarf_is_cie_32(self, v32)) goto end; //ignore cie
        cie_offset = tdcd_dwarf_adjust_cie_offset(self, cur_field_offset, (size_t)v32);
    }

    //get CIE
    cur_offset = self->memory_cur_offset;
    cie = tdcd_dwarf_get_cie_from_offset(self, cie_offset);
    ...
    if(0 != tdcd_dwarf_read_encoded(self, &v64, cie->fde_address_encoding)) goto end;
    pc_start = tdcd_dwarf_adjust_pc_from_fde(self, cur_field_offset, (uintptr_t)v64);
    ...
    {
        //get augmentation data length
        if(0 != tdcd_dwarf_read_uleb128(self, &v64)) goto end;
    ...
    if(NULL == (fde = malloc(sizeof(tdcd_dwarf_fde_t)))) goto end;
...
static tdcd_dwarf_fde_t *tdcd_dwarf_get_fde_no_hdr(tdcd_dwarf_t *self, uintptr_t pc)
{
    tdcd_dwarf_fde_t *fde = NULL;
    ...
    {
        if(NULL != (fde = tdcd_dwarf_get_fde_from_offset(self, &offset, pc))) break;
    }
    return fde;
}

static int tdcd_dwarf_get_fde_offset_from_pc(tdcd_dwarf_t *self, uintptr_t pc, size_t *fde_offset)
{
    int r;
    uint64_t v64;
    size_t first = 0;
    ...
    while(first < last)
    {
        ...
        if(0 != (r = tdcd_dwarf_read_encoded(self, &v64, self->eh_frame_hdr_table_encoding))) return r;
...
static tdcd_dwarf_fde_t *tdcd_dwarf_get_fde_with_hdr(tdcd_dwarf_t *self, uintptr_t pc)
{
    size_t offset;

    //get FDE-offset from PC
    if(0 != tdcd_dwarf_get_fde_offset_from_pc(self, pc, &offset)) return NULL;

    //get FDE from FDE-offset
    return tdcd_dwarf_get_fde_from_offset(self, &offset, pc);
}

static tdcd_dwarf_fde_t *tdcd_dwarf_get_fde(tdcd_dwarf_t *self, uintptr_t pc)
{
    switch(self->type)
    {
    case TDCD_DWARF_TYPE_DEBUG_FRAME:
    case TDCD_DWARF_TYPE_EH_FRAME:
        return tdcd_dwarf_get_fde_no_hdr(self, pc);
    case TDCD_DWARF_TYPE_EH_FRAME_HDR:
        return tdcd_dwarf_get_fde_with_hdr(self, pc);
    }
}
...
static tdcd_dwarf_loc_t *tdcd_dwarf_get_loc(tdcd_dwarf_t *self, tdcd_dwarf_fde_t *fde, uintptr_t pc)
{
    tdcd_dwarf_loc_t *loc = NULL, *loc_init = NULL, *loc_pc = NULL;
    ...
    tdcd_dwarf_cfa_t *cfa;
    uintptr_t operands[2];

    tdcd_dwarf_loc_node_t *loc_node, *loc_node_tmp;
    tdcd_dwarf_loc_node_stack_t loc_node_stack = TAILQ_HEAD_INITIALIZER(loc_node_stack);

    //start from instructions in CIE
    self->memory_cur_offset = cie_instr_start;

    if(NULL == (loc_init = calloc(1, sizeof(tdcd_dwarf_loc_t)))) return NULL;
    loc = loc_init;
    ...
    while(1)
    {
        ...
        if(loc == loc_init)
        {
            ...
            {
                if(NULL == (loc_pc = calloc(1, sizeof(tdcd_dwarf_loc_t)))) goto err;
                loc = loc_pc;
                ...
                memcpy(loc_pc, loc_init, sizeof(tdcd_dwarf_loc_t));
            }
        }
        else
        {
        ...
        case 0x2: //DW_CFA_offset
            if((uint16_t)cfa_op_ext >= TDCD_DWARF_REG_NUM) goto err;
            if(0 != tdcd_dwarf_read_uleb128(self, &v64)) goto err;
        ...
        case 0:
        default:
            cfa = &(tdcd_dwarf_cfa_table[cfa_op_ext]);
            ...
            for(i = 0; i < 2; i++)
            {
                ...
                {
                    if(0 != tdcd_dwarf_read_encoded(self, &v64, cfa->operand_types[i])) goto err;
                    operands[i] = (uintptr_t)v64;
                }
            }
            //extended opcode
            ...
            case 0x01: //DW_CFA_set_loc
                if(operands[0] < cur_pc) TDCD_LOG_WARN("DWARF: PC is moving backwards");
                cur_pc = operands[0];
                break;
            ...
            case 0x05: //DW_CFA_offset_extended
                if(operands[0] >= TDCD_DWARF_REG_NUM) goto err;
            ...
            case 0x0a: //DW_CFA_remember_state
                if(NULL == (loc_node = malloc(sizeof(tdcd_dwarf_loc_node_t)))) goto err;
                memcpy(&(loc_node->loc), loc, sizeof(tdcd_dwarf_loc_t));
                TAILQ_INSERT_HEAD(&loc_node_stack, loc_node, link);
                break;
            case 0x0b: //DW_CFA_restore_state
                if(NULL == (loc_node = TAILQ_FIRST(&loc_node_stack)))
                {
                    TDCD_LOG_WARN("DWARF: attempt to restore without remember");
                }
                else
                {
                    memcpy(loc, &(loc_node->loc), sizeof(tdcd_dwarf_loc_t));
...
static int tdcd_dwarf_eval_expression(tdcd_dwarf_t *self, tdcd_regs_t *regs, size_t offset, size_t end, uintptr_t *result)
{
    uint8_t         op_code;
    tdcd_dwarf_op_t *op;
    uintptr_t       operands[2];
...
#define pop()   ({if(stack_idx - 1 >= 64) return TDC_ERRNO_FORMAT; stack[--stack_idx];})
#define push(x) do{if(stack_idx >= 64) return TDC_ERRNO_NOSPACE; stack[stack_idx++] = (x);}while(0)
#define pick(n) ({if(stack_idx - 1 - n >= 64) return TDC_ERRNO_FORMAT; stack[stack_idx - 1 - n];})
    ...
    {
        if(j++ > 1000) return TDC_ERRNO_RANGE;
            
        //read operation code
        if(0 != (r = tdcd_dwarf_read_bytes(self, &op_code, 1))) return r;

        //get operation info
        op = &(tdcd_dwarf_op_table[op_code]);

        //check illegal operation code
        if(1 == op->illegal) return TDC_ERRNO_ILLEGAL;

        //check required number of stack elements
        if(op->required_stack_values > stack_idx) return TDC_ERRNO_MISSING;
        ...
        for(i = 0; i < 2; i++)
        {
            ...
            {
                if(0 != (r = tdcd_dwarf_read_encoded(self, &vu64, op->operand_types[i]))) return r;
        ...
        case 0x06: //DW_OP_deref
            if(0 != (r = tdcd_util_ptrace_read_fully(self->pid, pop(), &vup1, sizeof(vup1)))) return r;
            push(vup1);
            break;
        case 0x94: //DW_OP_deref_size
            if(0 == operands[0] || operands[0] > sizeof(uintptr_t)) return TDC_ERRNO_FORMAT;
            if(0 != (r = tdcd_util_ptrace_read_fully(self->pid, pop(), &vup1, operands[0]))) return r;
            push(vup1);
            break;
        ...
        case 0x15: //DW_OP_pick:
            if(operands[0] > stack_idx) return TDC_ERRNO_FORMAT;
        ...
        case 0x1b: //DW_OP_div
            vup1 = pop();
            vup2 = pop();
            if(0 == vup1) return TDC_ERRNO_FORMAT;
        ...
        case 0x8f: //DW_OP_breg31
            if(op_code - 0x70 >= TDCD_REGS_MACHINE_NUM) return TDC_ERRNO_FORMAT;
            push(regs->r[op_code - 0x70] + operands[0]);
            break;
        case 0x92: //DW_OP_bregx
            if(operands[0] >= TDCD_REGS_MACHINE_NUM) return TDC_ERRNO_FORMAT;
            push(regs->r[operands[0]] + operands[1]);
            break;
        case 0x96: //DW_OP_nop
            break;
        default:
            return TDC_ERRNO_ILLEGAL;
        }
    }

    if(0 == stack_idx) return TDC_ERRNO_FORMAT;
    *result = pick(0);
    return 0;
}
#pragma clang diagnostic pop
    
static int tdcd_dwarf_eval(tdcd_dwarf_t *self, tdcd_dwarf_fde_t *fde, tdcd_dwarf_loc_t *loc, tdcd_regs_t *regs, int *finished)
{
    int        r;
    size_t     i;
    uintptr_t  cfa;
    uintptr_t  value;
    tdcd_regs_t regs_orig = *regs;
    ...
    case DW_LOC_REGISTER:
        if(loc->cfa_rule.values[0] >= TDCD_REGS_MACHINE_NUM) return TDC_ERRNO_RANGE;
        cfa = (uintptr_t)(regs_orig.r[loc->cfa_rule.values[0]] + loc->cfa_rule.values[1]);
        break;
    case DW_LOC_VAL_EXPRESSION:
        if(0 != (r = tdcd_dwarf_eval_expression(self, &regs_orig, (size_t)(loc->cfa_rule.values[1] - loc->cfa_rule.values[0]),
    ...
    //regs
    for(i = 0; i < TDCD_REGS_MACHINE_NUM; i++)
    {
        switch(loc->reg_rules[i].type)
        {
        case DW_LOC_OFFSET:
            if(0 != (r = tdcd_util_ptrace_read_fully(self->pid, (uintptr_t)(cfa + loc->reg_rules[i].values[0]), &(regs->r[i]), sizeof(regs->r[i])))) return r;
            break;
        case DW_LOC_VAL_OFFSET:
            regs->r[i] = (uintptr_t)(cfa + loc->reg_rules[i].values[0]);
            break;
        case DW_LOC_REGISTER:
            if(loc->reg_rules[i].values[0] >= TDCD_REGS_MACHINE_NUM) return TDC_ERRNO_RANGE;
            regs->r[i] = (uintptr_t)(regs_orig.r[loc->reg_rules[i].values[0]] + loc->reg_rules[i].values[1]);
            break;
        case DW_LOC_EXPRESSION:
            if(0 != (r = tdcd_dwarf_eval_expression(self, &regs_orig, (size_t)(loc->reg_rules[i].values[1] - loc->reg_rules[i].values[0]),
                                                   (size_t)(loc->reg_rules[i].values[1]), &value))) return r;
            if(0 != (r = tdcd_util_ptrace_read_fully(self->pid, value, &(regs->r[i]), sizeof(regs->r[i])))) return r;
            break;
        case DW_LOC_VAL_EXPRESSION:
            if(0 != (r = tdcd_dwarf_eval_expression(self, &regs_orig, (size_t)(loc->reg_rules[i].values[1] - loc->reg_rules[i].values[0]),
                                                   (size_t)(loc->reg_rules[i].values[1]), &value))) return r;
    ...
    //step PC and SP
    tdcd_regs_set_pc(regs, return_address_undefined ? 0 : regs->r[fde->cie->return_address_register]);
    tdcd_regs_set_sp(regs, cfa);
    
    //if the pc was set to zero, consider this the final frame
    *finished = (0 == tdcd_regs_get_pc(regs) ? 1 : 0);
...
static int tdcd_dwarf_init_eh_frame_hdr(tdcd_dwarf_t *self)
{
    int      r;
    uint64_t v64;
    uint8_t  data[4];
    ...
    if(0 != (r = tdcd_dwarf_read_bytes(self, data, 4))) return r;
    version = data[0];
    ptr_encoding = data[1];
    ...
    //check version
    if(1 != version) return TDC_ERRNO_FORMAT;
    ...
    case DW_EH_PE_sleb128:
    default:
        return TDC_ERRNO_FORMAT; //using .eh_frame for linear search
    }
    
    //skip .eh_frame ptr
    self->memory_pc_offset = self->memory_cur_offset;
    if(0 != (r = tdcd_dwarf_read_encoded(self, &v64, ptr_encoding))) return r;

    //get fde count
    self->memory_pc_offset = self->memory_cur_offset;
    if(0 != (r = tdcd_dwarf_read_encoded(self, &v64, fde_count_encoding))) return r;
    if(0 == v64) return TDC_ERRNO_FORMAT;
                     ...
                     size_t offset, size_t size, tdcd_dwarf_type_t type)
{
    ...
    if(NULL == (*self = calloc(1, sizeof(tdcd_dwarf_t)))) return TDC_ERRNO_NOMEM;
    (*self)->type = type;
    (*self)->pid = pid;
    ...
    //for .eh_frame_hdr
    if(TDCD_DWARF_TYPE_EH_FRAME_HDR == type)
        if(0 != (r = tdcd_dwarf_init_eh_frame_hdr(*self))) goto err;
...
{
    tdcd_dwarf_fde_t *fde = NULL;
    tdcd_dwarf_loc_t *loc = NULL;
    int              r   = TDC_ERRNO_NOTFND;

    //find FDE & CIE from PC
    if(NULL == (fde = tdcd_dwarf_get_fde(self, pc)))
    {
#if TDCD_DWARF_DEBUG
        TDCD_LOG_DEBUG("DWARF: get FDE failed, step_pc=%"PRIxPTR, pc);
#endif
        goto end;
    }
    
    //find LOCATION in the FDE from PC
    if(NULL == (loc = tdcd_dwarf_get_loc(self, fde, pc)))
    {
#if TDCD_DWARF_DEBUG
        TDCD_LOG_DEBUG("DWARF: get LOC failed, step_pc=%"PRIxPTR, pc);
#endif
        goto end;
    }

    //eval the actual registers
    if(0 != (r = tdcd_dwarf_eval(self, fde, loc, regs, finished)))
    {
#if TDCD_DWARF_DEBUG
        TDCD_LOG_DEBUG("DWARF: eval failed, step_pc=%"PRIxPTR, pc);
#endif
        goto end;
    }
