...
static size_t tdcd_util_process_vm_readv(pid_t pid, uintptr_t remote_addr, void* dst, size_t dst_len)
{
...
static size_t tdcd_util_original_ptrace(pid_t pid, uintptr_t remote_addr, void *dst, size_t dst_len)
{
    ...
    if(align_bytes != 0)
    {
        if(0 != tdcd_util_ptrace_read_long(pid, remote_addr & ~(sizeof(long) - 1), &data)) return 0;

#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wgnu-statement-expression"
        size_t copy_bytes = TDCC_UTIL_MIN(sizeof(long) - align_bytes, dst_len);
#pragma clang diagnostic pop
    ...
    {
        if(0 != tdcd_util_ptrace_read_long(pid, remote_addr, &data)) return bytes_read;
...
size_t tdcd_util_ptrace_read(pid_t pid, uintptr_t remote_addr, void *dst, size_t dst_len)
{
    ...
    {
        size_t bytes = tdcd_util_process_vm_readv(pid, remote_addr, dst, dst_len);
        if(bytes > 0)
        {
            __atomic_store_n(&ptrace_read, tdcd_util_process_vm_readv, __ATOMIC_SEQ_CST);
            return bytes;
        }
        bytes = tdcd_util_original_ptrace(pid, remote_addr, dst, dst_len);
        if(bytes > 0)
        {
            __atomic_store_n(&ptrace_read, tdcd_util_original_ptrace, __ATOMIC_SEQ_CST);
            return bytes;
        }
        return 0;
    }
...
{
    size_t rc = tdcd_util_ptrace_read(pid, addr, dst, bytes);
    return rc == bytes ? 0 : TDC_ERRNO_MISSING;
}
...
int tdcd_util_ptrace_read_long(pid_t pid, uintptr_t addr, long *value)
{
    ...
    *value = ptrace(PTRACE_PEEKTEXT, pid, (void *)addr, NULL);
    if(-1 == *value && 0 != errno)
    {
        //TDCD_LOG_INFO("UTIL: ptrace error, addr:%"PRIxPTR", errno:%d\n", addr, errno);
        return errno;
    }
...
static void* tdcd_util_xz_alloc(ISzAllocPtr p, size_t size)
{
    (void)p;
    return malloc(size);
}

static void tdcd_util_xz_free(ISzAllocPtr p, void* address)
{
    (void)p;
    free(address);
}

static int tdcd_util_xz_crc_gen = 0;
int tdcd_util_xz_decompress(uint8_t* src, size_t src_size, uint8_t** dst, size_t* dst_size)
{
    ...
    ISzAlloc     alloc = {.Alloc = tdcd_util_xz_alloc, .Free = tdcd_util_xz_free};
    CXzUnpacker  state;
    ECoderStatus status;

    if(!tdcd_util_xz_crc_gen)
    {
        //call these initialization functions only once
        tdcd_util_xz_crc_gen = 1;
