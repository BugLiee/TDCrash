...
#define TDCC_MEMINFO_DELETE_STR     " (deleted)"
#define TDCC_MEMINFO_DELETE_STR_LEN 10
#define TDCC_MEMINFO_HEAD_FMT       "%13s %8s %8s %8s %8s %8s %8s %8s\n"
#define TDCC_MEMINFO_DATA_FMT       "%13s %8zu %8zu %8zu %8zu %8zu %8zu %8zu\n"
#define TDCC_MEMINFO_SUM_HEAD_FMT   "%21s %8s\n"
#define TDCC_MEMINFO_SUM_DATA_FMT   "%21s %8zu\n"
#define TDCC_MEMINFO_SUM_DATA2_FMT  "%21s %8zu %21s %8zu\n"
...
static const char *tdcc_meminfo_label[] =
        {
                "Native Heap",
                "Dalvik Heap",
                "Dalvik Other",
...
static void tdcc_meminfo_load(FILE *fp, tdcc_meminfo_t *stats, int *found_swap_pss)
{
    char       line[1024];
    size_t     len;
    size_t     temp;
    ...
    while(!done)
    {
        ...
        else
        {
            ...
            if(name_len > TDCC_MEMINFO_DELETE_STR_LEN &&
               0 == strcmp(name + name_len - TDCC_MEMINFO_DELETE_STR_LEN, TDCC_MEMINFO_DELETE_STR))
            {
                name_len -= TDCC_MEMINFO_DELETE_STR_LEN;
                name[name_len] = '\0';
            }
...
static int tdcc_meminfo_record_sys(int log_fd)
{
    return tdcc_util_record_sub_section_from(log_fd, "/proc/meminfo", " System Summary (From: /proc/meminfo)\n", 0);
}

static int tdcc_meminfo_record_proc_status(int log_fd, pid_t pid)
{
    char  path[64];
    snprintf(path, sizeof(path), "/proc/%d/status", pid);

    return tdcc_util_record_sub_section_from(log_fd, path, " Process Status (From: /proc/PID/status)\n", 0);
}

static int tdcc_meminfo_record_proc_limits(int log_fd, pid_t pid)
{
    char  path[64];
    snprintf(path, sizeof(path), "/proc/%d/limits", pid);

    return tdcc_util_record_sub_section_from(log_fd, path, " Process Limits (From: /proc/PID/limits)\n", 0);
}
...
int tdcc_meminfo_record(int log_fd, pid_t pid)
{
    char           path[64];
    FILE          *fp = NULL;
    tdcc_meminfo_t  stats[_NUM_HEAP];
    tdcc_meminfo_t  total;
    int            found_swap_pss = 0;
    size_t         i;
    int            r = 0;

    tdcc_libc_support_memset(stats, 0, sizeof(stats));
    tdcc_libc_support_memset(&total, 0, sizeof(total));

    //load memory info from /proc/pid/smaps
    snprintf(path, sizeof(path), "/proc/%d/smaps", pid);
    if(NULL == (fp = fopen(path, "r"))) return 0;
    tdcc_meminfo_load(fp, stats, &found_swap_pss);
    fclose(fp);
    ...
    //dump
    if(0 != (r = tdcc_util_write_str(log_fd, "memory info:\n"))) return r;
    if(0 != (r = tdcc_meminfo_record_sys(log_fd))) return r;
    if(0 != (r = tdcc_meminfo_record_proc_status(log_fd, pid))) return r;
    if(0 != (r = tdcc_meminfo_record_proc_limits(log_fd, pid))) return r;
    if(0 != (r = tdcc_util_write_str(log_fd, " Process Details (From: /proc/PID/smaps)\n"))) return r;
    if(0 != (r = tdcc_util_write_format(log_fd, TDCC_MEMINFO_HEAD_FMT, "", "Pss", "Pss", "Shared", "Private", "Shared", "Private", found_swap_pss ? "SwapPss" : "Swap"))) return r;
    if(0 != (r = tdcc_util_write_format(log_fd, TDCC_MEMINFO_HEAD_FMT, "", "Total", "Clean", "Dirty", "Dirty", "Clean", "Clean", "Dirty"))) return r;
    if(0 != (r = tdcc_util_write_format(log_fd, TDCC_MEMINFO_HEAD_FMT, "", "------", "------", "------", "------", "------", "------", "------"))) return r;
    for(i = 0; i < _NUM_EXCLUSIVE_HEAP; i++)
    {
        ...
        {
            if(0 != (r = tdcc_util_write_format(log_fd, TDCC_MEMINFO_DATA_FMT, tdcc_meminfo_label[i],
                                               stats[i].pss,
    ...
    if(0 != (r = tdcc_util_write_format(log_fd, TDCC_MEMINFO_DATA_FMT, "TOTAL",
                                       total.pss,
                                       total.swappable_pss,
                                       total.shared_dirty,
                                       total.private_dirty,
    ...
    if(0 != (r = tdcc_util_write_format(log_fd, TDCC_MEMINFO_SUM_HEAD_FMT, "", "Pss(KB)"))) return r;
    if(0 != (r = tdcc_util_write_format(log_fd, TDCC_MEMINFO_SUM_HEAD_FMT, "", "------"))) return r;
    if(0 != (r = tdcc_util_write_format(log_fd, TDCC_MEMINFO_SUM_DATA_FMT, "Java Heap:",
                                       stats[HEAP_DALVIK].private_dirty +
                                       stats[HEAP_ART].private_dirty + stats[HEAP_ART].private_clean))) return r;
    if(0 != (r = tdcc_util_write_format(log_fd, TDCC_MEMINFO_SUM_DATA_FMT, "Native Heap:",
                                       stats[HEAP_NATIVE].private_dirty))) return r;
    if(0 != (r = tdcc_util_write_format(log_fd, TDCC_MEMINFO_SUM_DATA_FMT, "Code:",
    ...
    if(0 != (r = tdcc_util_write_format(log_fd, TDCC_MEMINFO_SUM_DATA_FMT, "Stack:",
                                       stats[HEAP_STACK].private_dirty))) return r;
    if(0 != (r = tdcc_util_write_format(log_fd, TDCC_MEMINFO_SUM_DATA_FMT, "Private Other:",
    ...
    if(0 != (r = tdcc_util_write_format(log_fd, TDCC_MEMINFO_SUM_DATA_FMT, "System:",
                                       total.pss - total.private_dirty - total.private_clean))) return r;
    if(found_swap_pss)
    {
        if(0 != (r = tdcc_util_write_format(log_fd, TDCC_MEMINFO_SUM_DATA2_FMT,
                                           "TOTAL:",
                                           total.pss,
                                           "TOTAL SWAP PSS:",
